...
class SerieServiceImplTest : AnnotationSpec() {
    ...
    lateinit var serieEntity: SerieEntity
    ...
    fun setUp(){
        ...
        serieEntity = SerieEntity(serie.id, "The 100", "Série pós apocaliptica","Ficção-Cientifica", "Netflix")
    }

    @Test
    fun `should get serie by id`(){
        every {repository.getById(any())} answers {serieEntity}
    ...
    fun `should get all series`(){
        every {repository.getAll()} answers {listOf(serieEntity)}
